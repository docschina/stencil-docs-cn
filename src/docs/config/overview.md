...
[Web Workers](https://stencil.docschina.org/docs/web-workers) for those tasks.
...
* `async`: DOM read and writes are scheduled in the next frame to prevent layout thrashing.
  ...
  thread to lock-up, it's recommended to try [Web Workers](https://stencil.docschina.org/docs/web-workers)
...
* `immediate`: Makes writeTask() and readTask() callbacks to be executed synchronously. Tasks
  ...
  to lock-up, it's recommended to try [Web Workers](https://stencil.docschina.org/docs/web-workers).
